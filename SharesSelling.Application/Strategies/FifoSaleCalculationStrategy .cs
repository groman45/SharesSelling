...
{
    /// <summary>
    /// This class implements specific strategy (FIFO) for calculating stock sale metrics.
    /// </summary>
    public class FifoSaleStrategy : ISaleCalculationStrategy
	{
		public SaleResult CalculateProfit(List<StockLot> entities, int countSharesToSell, decimal price)
		{
			...
			foreach (StockLot lot in entities.OrderBy(e => e.Date)) // Sort to provide FIFO
			{
